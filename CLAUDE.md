...
2. `_build_sheet_prompt` / `_build_overview_prompt` — compact prompts preserving the anti-hallucination rules (hard numbers must be quoted verbatim, no fabricated periods, `chartjson` required at the end). Prompts require all arithmetic to be emitted as `{{calc: 表达式}}` markers; `_CalcStreamer`/`_eval_calc_markers` (safe ast-validated evaluator, split-chunk safe) replace them with results in-stream — the model never does mental math.
...
| `POST /analyze/followup` | **二次追问**: JSON `{session, question}` → SSE `{type: text|think|error|report_edit|done}`. Answers with the session's data context (hard numbers + file summaries) + conversation history via `_build_followup_prompt`; sessions kept in-memory (`_SESSIONS`, last 20). **聊天改报告**: 编辑类问题（含"改/调整/补充/删"等词）上下文带完整报告，模型输出`【报告修改】`块（`修改章节`+`新内容`），`_apply_report_edit` 按章节标题替换报告并推送 `report_edit` 事件（携带完整新报告），前端即时重渲染报告与中栏卡片；普通追问只带报告前 4000 字控制预填充 |
