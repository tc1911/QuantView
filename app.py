...
def _build_sheet_prompt(label, summary, hard, question):
    ...
    parts += [
        ...
        "=== 分析要求（强制执行） ===",
        "【结构与格式：以专业经营分析报告为蓝本】",
        "1. 以本工作表为分析单元，逐板块拆分并独立深挖，每个板块一个 ## 标题，标题采用蓝本编号格式「x.y 标题」"
        "（如「1.1 收入端：锂电拉动总量，氟化学增长乏力」），编号从 1.1 开始按板块顺序连续递增，不允许合并或一笔带过。"
        "分析思路必须贴合上方'用户问题'：问题明确指向的板块/指标深挖并优先呈现，与问题无关的指标只放数据表不展开讨论，"
        "小标题、结论与建议围绕问题展开，禁止输出与问题无关的模板化内容。",
        "2. 每个板块必须包含：数据概览表、关键指标对比表、变化分析（以'关键发现'总结开场）、业务解读、策略建议、风险提示；"
        "骨架固定是质量底线，六个部分必须各自独立成节（'变化分析与业务解读'这类合并写法不合格，需拆开），"
        "但各小节内容侧重必须按用户问题调整：问题涉及的业务写深写透，问题未涉及的维度简写；"
        "如果问题指向单一指标（如只看应收账款），该指标的分析必须放在板块最前并给出明确结论。",
        "3. 数字格式：金额/产量等数值一律千分位（如 184,800、45,200）；比率保留两位小数（24.5%）；"
        "同比/环比变动率用 ±x.x% 表述；倍数变化写「+72.4%」或「翻倍」。"
        "⚠️ x.xpp（百分点）只能用于本身是百分比/比率/占比类的指标（毛利率、净利率、ROE、费率、占比等）的差值，"
        "如毛利率-5.1pp；带量纲数值（万元/吨、吨、万元、天等非百分比指标，如单位成本 6.246 万元/吨）的变动"
        "一律用 ±x.x% 相对变动率（如单位成本同比 +16.9%），禁止把绝对差（0.903）写成 90.3pp。",
        "4. 表格采用蓝本格式：跨期对比表用「指标 | 2026H1 | 2025H2 | 2025H1 | 同比 | 环比」"
        "（只有两期数据时删掉缺失期列），数据概览表列出本板块全部核心指标（一行一个指标，含全部跨期列，不设行数上限）；"
        "对比类表格须含「判断/解读」列（如：销售额|同比|环比|产成品库存|库存同比|解读）；禁止为了控制篇幅删减数据行。",
        "5. 变化分析以'关键发现'开头总结本板块最重要的 1-3 个信号（用①…②…或'最突出的信号：…'句式），"
        "再逐项展开归因；结论直给（如'销售收缩与库存堆积同时发生，意味着订单需求走弱而生产计划未及时调整'）；"
        "每条发现必须引用至少 2 个具体数值（硬数字优先），禁止“某些指标”“部分数据”等模糊表述。",
        "6. 业务解读/策略建议/风险提示每节 1-2 段、每段 ≤100 字，且每段必须引用至少 1 个具体数值；"
        "策略建议必须可执行、尽量带量化目标（如'目标 3 个月内库存回落至 8,000 万元以内'）；禁止空泛论述。",
        "【硬数字铁律：禁止编造、必须原样引用】",
        "7. 硬数字必须原样引用，禁止修改或换算；引用时保留原始指标名，禁止改写成硬数字中未出现的指标名；"
        "报告中出现的任何数值必须能在硬数字中找到指标名+期别完全匹配的行，找不到出处的数值一律留空写'无数据'；"
        "禁止挪用其他指标行的数值充当历史期；禁止将同一组数字复制粘贴到多个期别列。",
        "8. 硬数字的表头/期别标签（如「锂电/氟化学/合并」「1个月内/1-2个月」）必须原样作为指标名引用，"
        "禁止自创标签替换（如禁止把'锂电周转天数 76.527'改写成'1个月内·天 76.527'）；"
        "跨板块共有的指标（成本明细、周转天数、库存、原材料等）引用时必须标注所属板块（氟化学/锂电/合并），"
        "禁止把甲板块的数值当作乙板块的（如氟化学的'直接材料 23300'与锂电的'直接材料 73000'是两套数据，不得混用）。"
        "⚠️ 本表含氟化学/锂电双板块数据（如采购的'氟化学原材料采购量·六****'与'锂电原材料采购量·六****'是"
        "两个板块各自独立的物料数据，数值完全不同）时，必须按板块拆分为独立小节分析"
        "（如「1.1 氟化学原材料采购分析」「1.2 锂电原材料采购分析」），每个小节只引用该板块前缀的硬数字，"
        "禁止把两个板块的指标混在一张表里对比、禁止用甲板块的数值解释乙板块的走势。",
        "9. 只有当期数据（无历史期标签）的指标严禁编造对比期或变动率，只能陈述当期数值；"
        "历史期为'无数据'的指标只陈述当期数值，禁止写出任何增幅/降幅；禁止把当期值复制到历史期列、禁止从其他指标行取值；"
        "硬数字中真实存在的期别数值（如2025H2、年初）必须被引用使用，禁止把存在的数据标为'无数据'；"
        "资产负债表的'年初'标签就是历史期对比值（等同于其他表的2025H2），必须使用；"
        "同一节内数据概览表与关键指标对比表必须一致，禁止自相矛盾。",
        "10. 变动率引用：硬数字中含'同比/环比'开头的行（如「同比(1个月内=-0.023)」）是工作表自带的变动率（小数），"
        "必须直接引用并转成百分比（-2.3%），禁止自行重新计算或改写标签；同比=与上年同期（2025H1）比较，"
        "环比=与上一期（2025H2）比较，两者不得互换；变化分析的对比期必须与上方对比表一致（以'2025H2/年初'列为基准），"
        "禁止改用'2025H1'等其他列计算变动率。",
        "11. 比率/占比/费率/收益率类硬数字是小数（如 费用率=0.083 表示 8.3%，税负率=0.120 表示 12%，"
        "收益率=0.018 表示 1.8%），引用时保持小数原样（0.083）或乘 100 写成百分比（8.3%），"
        "严禁写成'0.083%'这类不乘 100 的格式；自行计算的比率同样必须乘 100（如 22100/184800=12%，不是 0.12%）。",
        "12. 所有需要计算的地方（加总、占比、比率、变动率）一律输出 {{calc: 表达式}} 标记，服务端自动计算并替换为结果——"
        ...
        "合计必须逐项相加并当场验算至少两遍（写完重新加，发现不一致必须修正），每个合计结果必须能由列出的算式精确推导，"
        "禁止凭印象填合计值（有合计行的硬数字直接引用合计行）。",
        "13. ⚠️ 预警标记只用于变动幅度绝对值 >30% 的指标，不足 30% 时严禁出现'预警'字样或 ⚠️ 符号"
        "（费用、成本、收入、利润的任何指标变动不足 30% 时一律不得加 ⚠️），写完必须自查全文，"
        "发现 <30% 的指标带 ⚠️ 立即删除。",
        "14. 只有当期数据的表，历史期列必须写'无数据'；'基准/冲刺'是两档目标值不是时间序列，"
        "禁止当作期初/期末计算变动率（预算执行应对比实际值与预算目标，如实际销售额 vs 预算基准）；"
        "'预算金额/合同金额/已付金额'是项目执行阶段而非时间序列，禁止计算降幅，应解读为执行进度。",
        "【图表】",
        "15. 分析正文之后输出【图表数据】区块，每个板块 1 个（禁止重复或近似重复的图表），数据必须来自硬数字，禁止编造，格式：",
        "```chartjson",
        '[{"title": "图表标题", "type": "bar/pie/line/bar_h", "data": {"指标1": 数值, "指标2": 数值}}]',
        "```",
        "data 的键名必须与硬数字指标名一致且引号闭合（如\"碳乙**\": 6500，禁止输出缺闭合引号的键名，否则整块图表解析失败被丢弃）；"
        "data 的每个值必须是单个数字，禁止数组或对象（如\"六\": [5800, 6200] 会导致解析失败）；"
        "同一指标的多期对比用多个键（如\"碳乙(2026H1)\": 6500, \"碳乙(2025H2)\": 6000）；"
        "只有当期数据的指标只列当期数值，禁止用 0、重复值或其他指标行数值填充缺失期别。",
        "【输出纪律】",
        "16. 本工作表总字数（含表格）控制在 2000-2800 字，超出的部分优先删文字、不删数据；数据优先、文字精简：表格是主体、文字是辅助；"
        "与用户问题无关的板块只给数据表不写正文。",
        "17. 本任务是单轮一次性任务，必须一次输出完整分析；禁止向用户提问、禁止'是否继续'式交互、禁止中途停止等待确认；"
        "直接输出分析正文，禁止任何前言、思考过程、计划或解释性开头（如'好的''首先''我需要''让我们'等），第一个字必须是正文；"
        "禁止用省略号（...）、'依此类推'、'其余板块'等占位内容敷衍。",
        "18. 禁止使用 LaTeX 公式、$...$、$$...$$、\\frac、\\approx 等数学符号、引用块（> 符号）等格式，一律使用纯 Markdown 表格与文本。",
        "19. 同一小节的多个子节必须覆盖不同内容，禁止两个子节使用相同表格、相同数值或相同结论；内容雷同的子节必须合并为一个；"
        "每个小节最多 4 个子节，超过即失败。",
        "20. 禁止出现硬数字中不存在的科目名及数值。编造科目黑名单（一律禁止出现）：人工费用、办公费、招待费、差旅费、"
...
def _build_overview_prompt(question, sections):
    ...
    parts += [
        "",
        "=== 总览部分要求（以专业经营分析报告为蓝本，强制执行） ===",
        "【结构：按以下章节顺序组织】",
        "1. 「一、总体经营评价」：",
        "   - 「1.1 核心结论」：开篇直接回答用户问题（结论先行，一段话讲清问题的答案），概括本期经营特征"
        "（如'增收不增利'），点出最关键 2-3 个信号（增长引擎、利润质量、现金流），并给出杜邦视角的一句话归因；金额千分位。",
        "   - 「1.2 关键财务指标总览」：表格（指标|2026H1|2025H2|2025H1|同比|环比），至少包含：营业收入"
        "（含氟化学/锂电拆分）、净利润、毛利率（合并）、净利率、ROE、总资产周转率、权益乘数、资产负债率、流动比率、总资产，"
        "数据必须来自分项分析，禁止编造。⚠️ 每一行的三列数值与同比/环比必须与分项分析中同一指标的表格逐期别核对一致"
        "（2026H1/2025H2/2025H1 三列严格对应，同比=对2025H1、环比=对2025H2），禁止推算、禁止复制其他行的数值、"
        "禁止把期别列搞混（如把 2025H1 与 2025H2 互换）。",
        "2. 「二、跨板块联动分析」：各工作表之间的联动与勾稽（收入↔成本↔存货↔应收↔现金流）、整体经营画像、资源错配检测、"
        "前瞻预测；分析侧重必须跟随用户问题的指向，问题涉及的板块重点展开，与问题无关的内容从简。",
        "3. 「三、风险提示（按风险等级排序）」：编号风险表（编号|风险事项|关键数据|可能后果），编号 R1-Rn 连续，"
        "按「高风险（需管理层立即处置）/中风险（需专项跟踪）/关注项（持续观察）」分组排序，共 6-13 条；"
        "每条'关键数据'必须引用具体数值（含变动率），'可能后果'写清影响路径。",
        "4. 「四、改善建议（按业务板块分组）」：每组 1 个板块（如氟化学品业务/锂电业务/营运资金与资产管理/资金与现金流管理/"
        "汇率与税务管理/投资与预算管理，覆盖实际分析到的板块），每条建议以动词开头、带量化目标与责任建议"
        "（如'立即启动含氟**剂产销平衡专项：冻结超量排产，目标 3 个月内库存回落至 8,000 万元以内'）。",
        "5. 「五、具体工作计划」：两张表——「三季度（7-9月）重点行动清单」（时间|行动项|责任建议|量化目标，≥6 行）与"
        "「全年关键里程碑（至2026年12月）」（时间|里程碑|目标值，≥6 行）。",
        "6. 「附：数据口径与勾稽说明」：列出分项数据中口径存疑/勾稽异常的项目（如净利润>毛利、利息收入为负、"
        "H1实际+H2预算≠全年预算等），每条一句话说明并'建议核实'；分项数据冲突时以多数分项或资产负债分项为准，并在此标注。",
        "【铁律】",
        "7. 引用具体数值时必须与分项分析一致，禁止编造；禁止把分项中标注'无数据'的指标填数；"
        "数据无法支撑的维度要明确说明数据局限；金额千分位；比率/占比/费率类数值必须乘 100 写百分比（0.120=12%，严禁写成 0.12%）。",
        "8. 所有计算（占比/进度/增幅/比率）使用 {{calc: 表达式}} 标记，服务端自动计算替换，禁止自己心算；"
        "占比必须注明分母（如 {{calc: 103700/119700*100}}%）。",
        "9. 输出使用 Markdown，一级章用 ##（如「## 一、总体经营评价」），子节用 ###（如「### 1.1 核心结论」）；"
        "重点数字加粗；禁止 LaTeX、引用块（> 符号）、省略号占位。",
        "10. 末尾输出【图表数据】区块（2-4 张），展示总览关键对比，数据必须来自上述分项分析，禁止编造，格式：",
        "```chartjson",
        '[{"title": "图表标题", "type": "bar/pie/line/bar_h", "data": {"指标1": 数值, "指标2": 数值}}]',
        "```",
        "11. 总览控制在 2500-3500 字以内（含表格与图表区块），数据优先：关键对比用表格呈现，"
        "每段论述必须引用具体数值，禁止逐表复述分项分析内容，只做跨板块整合与结论。",
        "12. 直接输出总览正文，禁止任何前言、思考过程、计划或解释性开头；禁止向用户提问、禁止中途停止等待确认。",
    ]
    return "\n".join(parts)
