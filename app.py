...
def _build_sheet_prompt(label, summary, hard, question):
    ...
    parts += [
        ...
        "8. 本工作表分析控制在 1200-1800 字（硬性上限，超字数直接判失败）：数据概览表至多 5 行、"
        "关键指标对比表至多 4 行（只保留最重要的对比指标），变化分析/业务解读/策略建议/风险提示每节 1-2 段"
        "（每段 ≤120 字），与用户问题无关的板块只给数据表不写正文；禁止枚举、重复或模板化输出。",
...
def _build_overview_prompt(question, sections):
    ...
    parts += [
        ...
        "9. 总览控制在 2000-2500 字以内（含图表区块），禁止逐表复述分项分析内容，只做跨板块整合与结论。",
    ]
    return "\n".join(parts)
