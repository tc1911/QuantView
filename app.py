...
21. 同一小节的多个子节必须覆盖不同内容，禁止两个子节使用相同表格、相同数值或相同结论；内容雷同的子节必须合并为一个。
""")
    prompt_parts.append("")
...
def _build_sheet_prompt(label, summary, hard, question):
    ...
    parts += [
        ...
        "21. 硬数字中真实存在的期别数值（如2025H2）必须被引用使用，禁止把存在的数据标为'无数据'；"
        "同一节内数据概览表与关键指标对比表必须一致，禁止自相矛盾。",
        "22. 同一小节的多个子节必须覆盖不同内容，禁止两个子节使用相同表格、相同数值或相同结论；内容雷同的子节必须合并为一个。",
        "23. 分析正文之后输出【图表数据】区块，每个分析模块至少一张图表，数据必须来自硬数字，禁止编造，格式：",
        "```chartjson",
        '[{"title": "图表标题", "type": "bar/pie/line/bar_h", "data": {"指标1": 数值, "指标2": 数值}}]',
        "```",
    ]
