...
1. 数据概览表只列核心指标，最多 8 行，其余指标用"共N项"概括，禁止逐行罗列。
...
def _build_sheet_prompt(label, summary, hard, question):
    ...
    parts += [
        ...
        "7. 数据概览表只列核心指标（最多8行），其余指标用'共N项'概括，禁止逐行或逐行号区间罗列。",
        "8. 本工作表分析控制在 1500-3000 字，禁止枚举、重复或模板化输出。",
        "9. 变化分析/业务解读/策略建议/风险提示每节各写 1-3 段，禁止每个指标单独一段的逐项式输出。",
        "10. 图表数据必须与正文一致：只有当期数据的指标只列当期数值，禁止用0或占位值填充缺失期别。",
        "11. 分析正文之后输出【图表数据】区块，每个分析模块至少一张图表，数据必须来自硬数字，禁止编造，格式：",
        "```chartjson",
        '[{"title": "图表标题", "type": "bar/pie/line/bar_h", "data": {"指标1": 数值, "指标2": 数值}}]',
        "```",
    ]
